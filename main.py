...
class VKBot:
    ...
    def name(self, user_id):
        ...
        try:
            ...
            print(user_id, 'Ошибка получения токена, введите токен в переменную - user_token')
    ...
    def find_persons(self, user_id, offset, user_list, seen_users):
        ...
        for i in seen_user:
            if i['vk_id'] == tuple_person['vk_id']:
                sum+=1
        if sum == 0:
            self.write_msg(user_id, f'{tuple_person["first_name"]} {tuple_person["last_name"]}, ссылка - {tuple_person["vk_link"]}')
